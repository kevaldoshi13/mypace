...
which calculate the length of one particular string value. a single row function returns one result for every row of a queried table or view.

single rowe functions can be further grouped together by the data type of their arguements and return values.
for example , length relates to the string data type. funtion can be classified corresponding to different data type as:
string function: for string data type
numeric function: for number dta type
conversion of one data type to another.
date function : for data type

aggregate function

avg: return an average value of 'n', ignoring null values in a columnn.
...
example:
select min(curball)"minimum balance" from acct_master;
 
count(expr): returns the number of rows where xpr is not null,
syntax:
COUNT([<distinct>|<all>]<expr>)

example:
select count(acct_no)"no of accounts" from acct_mstr;
